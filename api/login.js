// /api/login.js
// Autentica setor (RH / Segurança / admin) e emite cookie de sessão (JWT HS256).
// Envs necessárias: SECRET_SESSION_KEY, RH_PASSWORD, SEGURANCA_PASSWORD, ADMIN_PASSWORD.
...
function signJWT(payload, secret, expSec = 12 * 60 * 60) {
  ...
  const data = `${h}.${p}`;
  const s = crypto.createHmac('sha256', secret).update(data).digest('base64')
...
export default async function handler(req, res) {
  try {
    ...
    if (!expected || String(senha) !== String(expected)) {
    ...
    // Cookie de sessão HttpOnly + Secure (somente HTTPS)
    const cookie = [
      `session=${token}`,
      'Path=/',
      'HttpOnly',
